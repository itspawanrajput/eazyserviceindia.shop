  ...
  return (
    <section className="py-20 bg-slate-50">
      <div className="max-w-3xl mx-auto px-4">
        <Editable id="faq-title" type="text">
          ...
          {FAQS.map((faq, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div 
                ...
                className="w-full px-6 py-5 text-left flex items-center justify-between font-bold text-slate-800 hover:text-blue-600 transition-colors cursor-pointer"
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
              ...
              >
                <Editable id={`faq-q-${idx}`} type="text">
                  <span>{faq.q}</span>
                </Editable>
              ...
              {openIndex === idx && (
                <div className="px-6 pb-5 text-slate-600 animate-in fade-in slide-in-from-top-2 duration-300">
                  <Editable id={`faq-a-${idx}`} type="text">
                    <p>{faq.a}</p>
                  </Editable>
                </div>
              )}
