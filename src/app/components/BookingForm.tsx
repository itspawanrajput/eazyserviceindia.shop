...
export default function BookingForm({ serviceAreas }: BookingFormProps) {
    const [isDetecting, setIsDetecting] = useState(false);
    const [detectedLocation, setDetectedLocation] = useState<string>("");
    const [manualLocation, setManualLocation] = useState(serviceAreas[0] || "Delhi");
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                try {
                    ...
                    const city = data.address?.city || data.address?.town || data.address?.state_district || "Delhi-NCR";
                    const matched = serviceAreas.find(area => city.toLowerCase().includes(area.toLowerCase()));
                    setDetectedLocation(matched || city);
                    setUseManual(false);
                } catch {
                    setUseManual(true);
                } finally {
            ...
            <div className="max-w-7xl mx-auto px-4 relative z-10">
                <div className="flex flex-col lg:flex-row items-center gap-16">
                    <div className="w-full lg:w-1/2 text-center lg:text-left">
                        <h2 className="text-4xl md:text-6xl font-black mb-6 leading-tight">Book Your AC Service <br /> in Delhi-NCR Today</h2>
                        <p className="text-xl text-blue-100 mb-8 max-w-lg mx-auto lg:mx-0">Fill in your details and our expert technician will contact you within 10 minutes to confirm your slot.</p>
                            ...
                            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                                ...
                                <div>
                                    ...
                                    {!useManual && !detectedLocation && !isDetecting ? (
                                        <button type="button" onClick={handleAutoLocation}
                                            ...
                                            <MapPin className="w-5 h-5 group-hover:scale-110 transition-transform" /> Detect My Current Location
                                        </button>
                                    ) : isDetecting ? (
                                        <div className="w-full flex items-center justify-center gap-3 p-4 bg-slate-50 border border-slate-100 rounded-xl text-slate-400 font-medium italic">
                                            <Loader2 className="w-5 h-5 animate-spin" /> Fetching your location...
                                        </div>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2">
                                            <div className="flex-1 flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-xl text-green-700 font-bold">
                                                <div className="flex items-center gap-2"><CheckCircle className="w-5 h-5" /><span>Detected: {detectedLocation}</span></div>
                                                <button type="button" onClick={() => setUseManual(true)} className="text-xs text-green-600 underline underline-offset-2 hover:text-green-800">Change</button>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="relative">
                                            ...
                                            <button type="button" onClick={handleAutoLocation} className="absolute right-4 top-1/2 -translate-y-1/2 text-blue-600 hover:text-blue-800 font-bold text-xs">Auto-Detect</button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    <select className="w-full p-4 rounded-xl bg-slate-100 border-none focus:ring-2 focus:ring-blue-500 transition-all">
                                        <option>Split AC</option><option>Window AC</option><option>Cassette AC</option><option>Other</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Service Type</label>
                                    <select className="w-full p-4 rounded-xl bg-slate-100 border-none focus:ring-2 focus:ring-blue-500 transition-all">
                                        <option>AC Repair</option><option>AC Cleaning</option><option>Installation / Uninstallation</option><option>Gas Refilling</option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <button type="submit" className="w-full bg-blue-600 text-white p-5 rounded-xl font-extrabold text-lg shadow-lg hover:bg-blue-700 transition-all flex items-center justify-center">
                                    Book Service Now <Send className="w-5 h-5 ml-3" />
                                </button>
                                <p className="text-center text-slate-400 text-sm mt-4 font-medium italic">* 1-Month Repeat Visit Guarantee Included</p>
                            </form>
                        </div>
                    </div>
                </div>
