...
export default function ServiceSection({ service, alternate, phoneLink }: Props) {
            ...
            <div className="max-w-7xl mx-auto px-4">
                <div className={`flex flex-col ${alternate ? 'md:flex-row' : 'md:flex-row-reverse'} items-stretch gap-8 lg:gap-16`}>
                    <div className="w-full md:w-1/2">
                        <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative group">
                            <div className="overflow-hidden rounded-[40px] md:rounded-[50px] shadow-xl aspect-[4/3] md:aspect-square lg:aspect-[4/3]">
                                <img src={service.image} alt={service.heading} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" referrerPolicy="no-referrer" />
                            </div>
                                    ...
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className="flex items-center gap-8 md:gap-12 pr-8 md:pr-12 shrink-0">
                                            <div className="flex items-center gap-1 shrink-0">
                                                <div className="w-3 h-3 bg-white rounded-full flex items-center justify-center shrink-0"><Check className="w-2 h-2 text-blue-600" /></div>
                    ...
                    <div className="w-full md:w-1/2 flex flex-col py-2">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex flex-col justify-between h-full">
                            <div>
                                ...
                                <div className="flex flex-wrap md:grid md:grid-cols-2 gap-x-6 gap-y-3 mb-6">
                                    {service.content.map((item, idx) => (
                                        <div key={idx} className="flex items-center group">
                                            ...
                                            <span className="bg-blue-50 md:bg-transparent text-blue-700 md:text-slate-700 px-2 py-0.5 md:px-0 md:py-0 rounded-md md:rounded-none text-[9px] md:text-xs uppercase font-black tracking-wider border border-blue-100 md:border-none">{item}</span>
                                        </div>
                                    ))}
                                </div>
                                {service.description && (
                                    <p className="text-slate-500 text-xs md:text-[15px] leading-relaxed max-w-lg">{service.description}</p>
                                )}
                            </div>
                            <div className="flex flex-col sm:flex-row gap-4 mt-8">
                                <motion.a href="#booking-form" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                                    ...
                                    Book Now <ArrowRight className="ml-2 w-3.5 h-3.5 md:w-4 md:h-4" />
                                </motion.a>
                                <motion.a href={phoneLink} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                                    ...
                                    <Phone className="w-3 h-3 md:w-4 md:h-4 mr-2 text-blue-600 group-hover:text-white transition-colors" /> Call Now
                                </motion.a>
                            </div>
                        </motion.div>
                    </div>
