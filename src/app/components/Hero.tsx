...
import { BrandData, ServiceData } from '@/lib/data';
...
export default function Hero({ brand, services }: HeroProps) {
                    ...
                    <div className="w-full md:w-1/2 px-4 md:px-0 order-2 md:order-1 md:pt-8">
                        ...
                        <div className="flex items-center space-x-2 mb-8">
                            <div className="flex items-center bg-yellow-400/20 px-1.5 py-0.5 rounded">
                                ...
                                <span className="font-bold text-sm text-slate-800">{brand.heroRating}</span>
                            </div>
                            <span className="text-slate-500 text-sm">({brand.heroBookingCount})</span>
                        </div>

                        <div className="grid grid-cols-4 gap-2 mb-10">
                            {services.map((service) => (
                                <button key={service.id} onClick={() => scrollTo(service.id)} className="flex flex-col items-center group relative pt-4">
                                    {service.badge && (
                                        <div className="absolute top-0 left-1/2 -translate-x-1/2 z-10">
                                            <span className="bg-[#FFF1DB] text-[#D19234] text-[8px] md:text-[9px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap border border-[#F6E2C2] shadow-sm">{service.badge}</span>
                                        </div>
                                    )}
                                    ...
                                    <span className="text-[10px] md:text-[12px] font-bold text-center text-[#464646] leading-tight px-1 group-hover:text-blue-600">
                                        {service.title}
                                    </span>
                                </button>
                            ))}
                    ...
                    <div className="w-full md:w-1/2 order-1 md:order-2 px-4 md:px-0">
                        <div className="relative aspect-[16/10] rounded-2xl md:rounded-3xl overflow-hidden bg-[#78544B] shadow-2xl group">
                            <div className="absolute inset-0 z-0">
                                <img src={brand.heroBackgroundImage} alt="Background" className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
                            </div>
                            <img src={brand.heroPersonImage} alt="AC Expert" className="absolute right-0 top-0 h-full w-3/5 object-cover z-10 scale-110 group-hover:scale-115 transition-transform duration-700"
                                style={{ clipPath: 'polygon(20% 0, 100% 0, 100% 100%, 0% 100%)' }} referrerPolicy="no-referrer" />
                            <VIPBanner text={brand.vipText} />
                        </div>
                    </div>
                </div>
            </div>
...
function VIPBanner({ text }: { text: string }) {
    return (
        <div className="absolute bottom-0 left-0 right-0 bg-black/90 backdrop-blur-md p-3 md:p-4 flex items-center justify-between z-40 border-t border-white/10">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[#FFDC64] text-[8px] md:text-[10px] font-medium leading-tight">{text}</p>
                </div>
            </div>
            <button className="bg-white text-black font-bold px-4 py-1.5 rounded-lg text-[10px] md:text-xs hover:bg-slate-100 transition-colors shadow-lg active:scale-95">BUY</button>
        </div>
    );
}

function ServiceIcon({ serviceId }: { serviceId: string }) {
    if (serviceId.includes('install')) {
        return (
            <svg width="40" height="40" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <g transform="translate(48, 40)">
                    ...
                    {[0, 90, 180, 270].map(deg => (<rect key={deg} x="-1.5" y="-7" width="3" height="3" fill="#FD3752" transform={`rotate(${deg})`} />))}
                </g>
            </svg>
        );
    }
    if (serviceId.includes('cleaning')) {
        return (
            <svg width="40" height="40" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        return (
            <svg width="40" height="40" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <g transform="translate(18, 34)">
                    ...
                    {[0, 90, 180, 270].map(deg => (<rect key={deg} x="-18" y="-3" width="6" height="6" rx="1" fill="#FD3752" transform={`rotate(${deg})`} />))}
                </g>
            </svg>
        );
    }
